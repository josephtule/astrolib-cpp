...
void run_world_embedded_rk_diag() {
    ...
    const array<IntegratorTypeAdaptive, 9> methods_embedded{
        ...
        IntegratorTypeAdaptive::stepanov54,
    };
...
void run_ephemeris_io_diag() {
    ...
    const std::filesystem::path root
        = std::filesystem::temp_directory_path()
          / ("astrolib_ephemeris_io_diag_" + std::to_string(stamp));
    const std::filesystem::path cart_manifest = root / "cartesian.json";
    const std::filesystem::path cart_data = root / "cartesian.csv";
    const std::filesystem::path att_manifest = root / "orientation.json";
    const std::filesystem::path att_data = root / "orientation.csv";

    std::error_code ec;
    std::filesystem::create_directories(root, ec);
    if (ec) {
        std::println("Temporary Directory Status = false");
        return;
    }
    ...
    auto check_bool = [&](const string& name, bool ok) {
        ++checks;
        passed += ok;
        std::println("{} = {}", name, ok);
    };
    ...
    auto read_text = [](const std::filesystem::path& filepath, string& out) {
    ...
    EphemerisWriteOptions cart_opts{
        .overwrite = false,
        .precision = 17,
        .csv = EphemerisCSVLayout{.filepath = cart_data.filename().string()}
    };
    EphemerisWriteOptions att_opts{
        .overwrite = false,
        .precision = 17,
    ...
    status = load_cartesian_ephemeris(cart_manifest.string(), cart_loaded);
    check_status("Cartesian Load Status", status, StatusCode::ok);

    f64 cart_dt_error = 0.0;
    f64 cart_r_error = 0.0;
    f64 cart_v_error = 0.0;
    if (cart_loaded.dt.size() == cart.dt.size()
        ...
        for (i32 i = 0; i < cart.dt.size(); ++i) {
            ...
            cart_v_error = std::max(
                cart_v_error,
                (cart.states[i].v - cart_loaded.states[i].v).norm()
            );
        }
    } else {
        cart_dt_error = inf<f64>;
        cart_r_error = inf<f64>;
        cart_v_error = inf<f64>;
    }
    ...
    status = save_orientation_ephemeris(att_manifest.string(), att, att_opts);
    check_status("Orientation Save Status", status, StatusCode::ok);

    OrientationEphemerisTable att_loaded;
    status = load_orientation_ephemeris(att_manifest.string(), att_loaded);
    check_status("Orientation Load Status", status, StatusCode::ok);

    f64 att_dt_error = 0.0;
    f64 att_q_error = 0.0;
    f64 att_w_error = 0.0;
    if (att_loaded.dt.size() == att.dt.size()
        && att_loaded.states.size() == att.states.size()) {
        for (i32 i = 0; i < att.dt.size(); ++i) {
            att_dt_error = std::max(att_dt_error, std::abs(att.dt[i] - att_loaded.dt[i]));
            att_q_error = std::max(
                att_q_error,
                std::min(
                    (att.states[i].q - att_loaded.states[i].q).norm(),
                    (att.states[i].q + att_loaded.states[i].q).norm()
                )
            );
            att_w_error = std::max(
                att_w_error,
                (att.states[i].w - att_loaded.states[i].w).norm()
            );
        }
    } else {
        att_dt_error = inf<f64>;
        att_q_error = inf<f64>;
        att_w_error = inf<f64>;
    }
    const bool att_metadata_ok
        = att_loaded.metadata.frame.object == att.metadata.frame.object
          && att_loaded.metadata.frame.source_frame == att.metadata.frame.source_frame
    ...
    nlohmann::json cart_root;
    nlohmann::json att_root;
    const bool manifests_read
        = read_json(cart_manifest, cart_root) && read_json(att_manifest, att_root);
    check_bool("Saved Manifest Read", manifests_read);
    if (!manifests_read) return;

    auto make_manifest_case = [&](nlohmann::json root_case,
                                  const std::filesystem::path& manifest,
                                  const std::filesystem::path& data) {
        root_case["data"]["filepath"] = data.filename().string();
        return write_json(manifest, root_case);
    };

    const std::filesystem::path malformed_manifest = root / "malformed_columns.json";
    const std::filesystem::path malformed_data = root / "malformed_columns.csv";
    check_bool(
        "Malformed Column Fixture",
        make_manifest_case(cart_root, malformed_manifest, malformed_data)
            && write_text(malformed_data, "dt,rx,ry,rz,vx,vy,vz\n0,1,2\n")
    );
    CartesianEphemerisTable cart_failed;
    status = load_cartesian_ephemeris(malformed_manifest.string(), cart_failed);
    check_status("Malformed Column Status", status, StatusCode::parse_failed);

    const std::filesystem::path nonmono_manifest = root / "nonmonotonic.json";
    const std::filesystem::path nonmono_data = root / "nonmonotonic.csv";
    check_bool(
        "Non-Monotonic Fixture",
        make_manifest_case(cart_root, nonmono_manifest, nonmono_data)
            && write_text(
                nonmono_data,
                "dt,rx,ry,rz,vx,vy,vz\n"
                "0,7000,0,0,0,7.5,0\n"
                "0,6999,1,0,-0.1,7.5,0\n"
            )
    );
    status = load_cartesian_ephemeris(nonmono_manifest.string(), cart_failed);
    check_status("Non-Monotonic Status", status, StatusCode::non_monotonic_time);

    const std::filesystem::path nonfinite_manifest = root / "nonfinite.json";
    const std::filesystem::path nonfinite_data = root / "nonfinite.csv";
    check_bool(
        "Non-Finite Fixture",
    ...
    const std::filesystem::path zero_q_manifest = root / "zero_quaternion.json";
    const std::filesystem::path zero_q_data = root / "zero_quaternion.csv";
    check_bool(
    ...
    check_status("Zero Quaternion Status", status, StatusCode::invalid_att_state);

    nlohmann::json wrong_domain_root = cart_root;
    wrong_domain_root["domain"] = "orientation";
    const std::filesystem::path wrong_domain_manifest = root / "wrong_domain.json";
    ...
    nlohmann::json bad_frame_root = att_root;
    bad_frame_root["metadata"]["frame"]["target_frame"] = "";
    const std::filesystem::path bad_frame_manifest = root / "bad_frame.json";
    check_bool("Bad Frame Fixture", write_json(bad_frame_manifest, bad_frame_root));
    status = load_orientation_ephemeris(bad_frame_manifest.string(), att_failed);
    check_status("Bad Frame Status", status, StatusCode::invalid_ephemeris_metadata);

    nlohmann::json bad_convention_root = att_root;
    bad_convention_root["metadata"]["convention"]["rotation"] = "unknown";
    const std::filesystem::path bad_convention_manifest = root / "bad_convention.json";
    check_bool(
        "Bad Convention Fixture",
        write_json(bad_convention_manifest, bad_convention_root)
    );
    status = load_orientation_ephemeris(bad_convention_manifest.string(), att_failed);
    check_status("Bad Convention Status", status, StatusCode::unsupported_type);

    nlohmann::json missing_data_root = cart_root;
    missing_data_root["data"]["filepath"] = "missing.csv";
    const std::filesystem::path missing_data_manifest = root / "missing_data.json";
    check_bool(
        "Missing CSV Fixture",
        write_json(missing_data_manifest, missing_data_root)
    );
    status = load_cartesian_ephemeris(missing_data_manifest.string(), cart_failed);
    check_status("Missing CSV Status", status, StatusCode::file_not_found);

    status = save_cartesian_ephemeris(cart_manifest.string(), cart, cart_opts);
    check_status("Overwrite Disabled Status", status, StatusCode::file_already_exists);

    cart_opts.overwrite = true;
    status = save_cartesian_ephemeris(cart_manifest.string(), cart, cart_opts);
    check_status("Overwrite Enabled Status", status, StatusCode::ok);

    const std::filesystem::path directory_destination = root / "directory_destination";
    std::filesystem::create_directory(directory_destination, ec);
    status = save_cartesian_ephemeris(directory_destination.string(), cart, cart_opts);
    check_status("Directory Destination Status", status, StatusCode::invalid_input);

    EphemerisWriteOptions same_path_opts = cart_opts;
    same_path_opts.csv.filepath = "same.json";
    status = save_cartesian_ephemeris(
        (root / "same.json").string(),
        cart,
        same_path_opts
    );
    check_status("Identical Destination Status", status, StatusCode::invalid_input);

    string manifest_before;
    string data_before;
    const bool baseline_read
        = read_text(cart_manifest, manifest_before) && read_text(cart_data, data_before);
    check_bool("Unchanged Output Baseline Read", baseline_read);

    CartesianEphemerisTable invalid_cart = cart;
    invalid_cart.states.front().r(0) = std::numeric_limits<f64>::quiet_NaN();
    status = save_cartesian_ephemeris(cart_manifest.string(), invalid_cart, cart_opts);
    check_status("Invalid Table Save Status", status, StatusCode::invalid_state);

    string manifest_after;
    string data_after;
    const bool unchanged_output
        = read_text(cart_manifest, manifest_after) && read_text(cart_data, data_after)
          && manifest_before == manifest_after && data_before == data_after;
    check_bool("Invalid Save Preserved Outputs", unchanged_output);

    bool sidecars_found = false;
    for (const auto& entry : std::filesystem::directory_iterator(root)) {
        const string filename = entry.path().filename().string();
        if (filename.find(".astrolib-tmp-") != string::npos
            || filename.find(".astrolib-backup-") != string::npos) {
            sidecars_found = true;
            break;
        }
    }
    check_bool("Temporary and Backup Cleanup", !sidecars_found);
