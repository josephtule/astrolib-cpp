...
void run_ephemeris_io_diag() {
    ...
    const std::filesystem::path cart_manifest = valid_dir / "cartesian.json";
    const std::filesystem::path cart_data = valid_dir / "cartesian.csv";
    const std::filesystem::path att_manifest = valid_dir / "orientation.json";
    const std::filesystem::path att_data = valid_dir / "orientation.csv";

    std::error_code ec;
    std::filesystem::remove_all(root, ec);
    if (!ec) std::filesystem::create_directories(valid_dir, ec);
    if (!ec) std::filesystem::create_directories(invalid_dir, ec);
    if (!ec) std::filesystem::create_directories(save_safety_dir, ec);
    if (ec) {
        std::println("Ephemeris Diagnostic Directory Status = false");
        return;
    }
    std::println("Ephemeris Diagnostic Directory = {}", root.string());
    ...
    const std::filesystem::path nonmono_manifest = invalid_dir / "nonmonotonic.json";
    const std::filesystem::path nonmono_data = invalid_dir / "nonmonotonic.csv";
    check_bool(
    ...
    const std::filesystem::path nonfinite_manifest = invalid_dir / "nonfinite.json";
    const std::filesystem::path nonfinite_data = invalid_dir / "nonfinite.csv";
    check_bool(
        "Non-Finite Fixture",
    ...
    const std::filesystem::path bad_frame_manifest = invalid_dir / "bad_frame.json";
    ...
    const std::filesystem::path bad_convention_manifest
        = invalid_dir / "bad_convention.json";
    check_bool(
    ...
    const std::filesystem::path directory_destination
        = save_safety_dir / "directory_destination";
    ...
    status = save_cartesian_ephemeris(
        (save_safety_dir / "same.json").string(),
        cart,
        same_path_opts
    );
