...
struct BodyEphemerisTable {
    ...
    // TODO: add frame/time metadata, assume sim inertial for now
};

inline StatusCode validate_body_ephemeris_table(const BodyEphemerisTable& table) {
    if (table.t.size() != table.x_tr.size()) return StatusCode::size_mismatch;

    return StatusCode::ok;
}

inline StatusCode provider_body_tr(
    const BodyEphemerisTable& table,
    EntityId body_id,
    f64 t,
    StateTr& out,
    f64 tol = tol12
    ...
    StatusCode status = validate_body_ephemeris_table(table);
    // if (!od_status_success(status)) {
    //     return status;
    // }
