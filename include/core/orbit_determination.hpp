...
inline bool iod_vec_valid(ecref<vec3d> v, f64 tol = tol_strict) {
    return v.allFinite() && v.norm() > tol;
}

inline bool iod_time_valid(f64 t1, f64 t2, f64 t3, f64 tol = tol_strict) {
    return std::isfinite(t1) && std::isfinite(t2) && std::isfinite(t3)
           && std::abs(t1 - t2) > tol && std::abs(t3 - t2) > tol
           && std::abs(t3 - t1) > tol;
}

inline IODAnglesObs3 iod_angles3_from_radec(
...
inline IODResult iod_gauss(const IODAnglesObs3& arc, f64 mu, f64 tol = tol_med) {
    return iod_gauss(arc.t, arc.L, arc.R, mu, tol);
}

inline IODResult iod_gibbs(
    ecref<vec3d> r1,
    ecref<vec3d> r2,
    ecref<vec3d> r3,
    f64 mu,
...
inline IODResult iod_herrickgibbs(
    f64 t1,
    f64 t2,
    f64 t3,
    ecref<vec3d> r1,
